...
```{r, echo=F, message=F}
knitr::opts_chunk$set(echo = TRUE,  results = FALSE, eval = F, message = FALSE, message = FALSE, warning = FALSE)
library(printr)
```
...
For this tutorial, we are again going to load the `tidyverse`, `tidytext` (for some visualizations), `glue` (useful for glueing together different texts and objects) and a new package called `tidyllm`. This last package provides function for working with LLMs, whether local (via ollama) or via API (e.g., GPT from OpenAI). 
...
Whereas the first three are available via CRAN and can be installed with `install.packages()`, we do want to install the latest development version, which has a slightly new framework. We can do this via the `remotes` package (see code below). 
...
**Note:**  In the code below, please make sure to load the `rollama` model, pull the model that you want to download and then again `detach` the `rollama` package as it otherwise overwrites a function from the `tidyllm` package that we need. 
