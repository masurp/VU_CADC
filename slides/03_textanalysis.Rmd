...
subtitle: "Week 3: Basics of Automatic Text Analysis and Dictionary Approaches"
author: ""
institute: ""
...
```{r plot-label-out1, ref.label="plot_speech", echo=FALSE}
...
dtm1 <- corp %>%
  ...
  dfm 
# textplot_wordcloud(dtm1, max_words = 50)
```
...
# Line plot
d %>%
  ggplot(aes(x = Date, y = sentiment2)) +
  geom_line(color = "darkblue") +
  theme_economist() +
  ggtitle("Sentiment (V2) in SOTU-speeches over time")
```
...
```{r, echo = F, fig.width=14, fig.height=5.5}
# Line plot
d %>%
...
d %>%
  filter(President %in% c("Obama", "Trump", "Clinton")) %>%
  pivot_longer(sentiment1:subjectivity) %>%
  ggplot(aes(x = Date, y = value, color = name)) +
  geom_line(size = 1.5) +
  geom_point(size = 3) +
  scale_color_brewer(palette = "Set1") +
  facet_grid(name~President, scales = "free") +
  theme_bw() +
...
- To get an overall quantitative measure of the validity of a dictionary, one should always manually code a random sample of documents and compare the coding with the dictionary results
...
- General procedure
    ...
    - Code actual sentiment with human coders (the 'gold standard')
    - Combine the manual coding results with the results from the automated sentiment analysis
    - Produce validation scores, e.g., correlation between manual and automated coding, validation scores

---

# Typical validation scores

- Accuracy: Percentage of agreement between automated algorithm and gold standard
