...
<span style="font-size:0.6em;">Hilbert & Lopez, 2011</span>
...
**Note:** Although valuable, this measurements can be biased (Scharkow, 2013; Parry et al., 2021)!
...
This is what we will learn in this course!
...
3.   have knowledge about different approaches of text classification (incl. machine learning, large language models...)
...
-   gather, potentially scrape, and import data from different file types, APIs, and websites
...
-   use different approaches (e.g., dictionary, classical machine learning, transformer, LLMs) to extract information from textual data
...
5. The general text classification pipeline
...
<span style="font-size:0.65em;">Blumenstock, Cadamura, & On, 2015</span>
...
<span style="font-size:0.65em;">Salganik, 2017, chap. 2.3</span>
...
- Social Media Use
    ...
    - Tracking of social media use and relationships with developmental and health outcome
...
## Example 2: Dutch Telegramsphere
...
<span style="font-size:0.65em;">Topic similarity network</span>
...
Why computational methods and big data can be unethical.
...
<span style="font-size:0.65em;">Kramer et al., 2014</span>
...
## Ethical challenges with computational methods
...
- Tend to reproduce Biases

    - Pre-trained models are known to reproduce bias and discrimination
    - Large language models (such as GPT) can hallucinate
    - Need to be careful in using such methods for our research
    - Validation of utmost importance!

. . .

-  In case of commercial models, privacy of data subjects

    - Should we really share personal information with large tech companies?
    - Informed consent technically necessary, but often unattainable
...
## Ethical principles for computational research

1. **Ownership**: We need to respect that individuals "own" their data

2. **Transparency**: If an individual's personal data is used, they should have transparent access to (information about) the algorithm design used to generate aggregate data sets and be informed about potential harm that can arise from the research (e.g., bias, discrimination)
    
3. **Consent:** If an individual or legal entity would like to use personal data, one needs informed and explicitly expressed consent of what personal data moves to whom, when, and for what purpose from the owner of the data (if this is not possible, the risks/benefits profile of a study needs to be acceptable)

4. **Privacy and Data Security:** If data transactions occur all reasonable effort needs to be made to preserve privacy and security, particularly of sensitive personal data
...
6. **Openness:** Whenever possible (see point 4), aggregate data sets should be freely available. 

<span style="font-size:0.65em;">Salganik, 2018, chap. 6</span>
...
|:------ |:-------------------|
| Ambigous sentence | "Include Your Children When Baking Cookies" |
| Word tokens | "Include", "Your", "Children", "When", "Baking",  "Cookies" |
| Bi-grams | "Include", "Your Children", "When", "Baking Cookies" |
| 3-grams | "Include Your Children", "When Baking Cookies" |
...
![](img/text_analysis_fundamentals/Slide1.png)

- Goal is generally to label (or annotate) previously unlabeled text

- Although the specific methods may change, the steps usually remain the same
...
-   Feature engineering is the process of selecting, manipulating, <br>
    and transforming raw data into so-called features 

-   What type of feature engineering is necessary or useful <br> strongly 
    depends on the method used, but may include
...
![](img/text_analysis_fundamentals/validation.png){.absolute top=10 right=0 width="300"}
...
dictionary <- tibble(movie = c("film", "movie", "director", "actor"),
                     methods = c("methods", "classification", "analysis", "science"))
print(dictionary)
...
**Office Hours:** Mondays and Wednesdays 14.00 to 16.00 on Zoom (see canvas)
...
-   Students are required to hand in the RMarkdown file and a compiled html on Thursday (24:00, hard deadline!) the week after. All homework assignments must be submitted to pass.
...
Students are required to hand in the slides and analyses beforehand (Deadline: 17th of December 2021 at 24:00, hard deadline!)
